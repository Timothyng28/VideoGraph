...
def remove_transcription_params(code: str, voice_id: str = None) -> str:
    ...
    for service_name in ['ElevenLabsService', 'ElevenLabsTimedService']:
        ...
        code = re.sub(
            ...
            f'{service_name}(voice_id="{selected_voice_id}"',
            code
        )
        ...
        code = re.sub(
            rf'{service_name}\(\s*,?\s*\)',
            f'{service_name}(voice_id="{selected_voice_id}", transcription_model=None)',
            code
        )

        # Pattern 4b: Add transcription_model=None if only voice_id is present
        code = re.sub(
            rf'{service_name}\(voice_id="{selected_voice_id}"\)',
            f'{service_name}(voice_id="{selected_voice_id}", transcription_model=None)',
            code
        )
        ...
        code = re.sub(
            rf'{service_name}\(voice_id="{selected_voice_id}",\s*model="[^"]*",?\s*',
            f'{service_name}(voice_id="{selected_voice_id}", ',
            code
        )
    ...
    code = re.sub(
        r'GTTSService\([^)]*\)',
        f'ElevenLabsTimedService(voice_id="{selected_voice_id}", transcription_model=None)',
        code
    )
