...
import { evaluateAnswer, generateLeafQuestion } from "../services/llmService";
...
import {
  ...
  GenerationProgress,
  SectionDetail,
} from "../services/videoRenderService";
import {
  addChildNode,
  addRootNode,
  getChildren,
...
export const VideoController: React.FC<VideoControllerProps> = ({
  ...
  const [quizCorrectAnswer, setQuizCorrectAnswer] = useState<string | null>(
    null
  );
  ...
  const currentNodeNumber = currentNode
    ? getNodeNumber(session.tree, currentNode.id)
    : "";
  ...
  useEffect(() => {
    ...
    const currentNode =
      session.tree.nodes.size > 0 ? getCurrentNode(session.tree) : null;
    ...
    if (!currentNode && !isGenerating) {
      console.log(
        "Generating initial segment for topic:",
        session.context.initialTopic
      );
  ...
  const generateInitialSegment = async () => {
    ...
    try {
      ...
      const result = await generateVideoScenes(
        topic,
        (progress) => {
  ...
  const requestNewTopic = useCallback(
    async (newTopic: string, imageData?: string, imageFilename?: string) => {
      ...
      if (imageData) {
        console.log(`[${request.id}] Image context provided`);
      }
      ...
      try {
        ...
        if (result.success && result.sections && result.sections.length > 0) {
          ...
          // Map section URLs to VideoSegments
          const newSegments: VideoSegment[] = result.sections.map(
            (sectionUrl, index) => {
              const sectionMatch = sectionUrl.match(/section_(\d+)\.mp4/);
              const detail = detailByUrl.get(sectionUrl);
              const sectionNum =
          ...
          setMostRecentGenerationId((current) => {
            if (current === request.id) {
              console.log(
                `[${request.id}] Most recent request - navigating to new topic`
              );
              navigateToNodeHelper(session.tree, newRootNode.id);
            } else {
              console.log(
                `[${request.id}] Not most recent (current: ${current}) - skipping navigation`
              );
            }
            return current;
          });
          ...
          console.log(
            `[${request.id}] Created new root tree with ${newSegments.length} video segments for topic: ${newTopic}`
          );
        } else {
          const errorMsg =
            result.error || "Failed to generate video scenes for new topic";
          updateGenerationRequest(request.id, {
            status: "error",
            error: errorMsg,
          });
   ...
   * Now supports parallel generation and image context
   */
  const handleQuestionBranch = useCallback(
    async (question: string, imageData?: string, imageFilename?: string) => {
      // Check for cached session match first
      ...
      if (processingQuestionsRef.current.has(questionKey)) {
        console.warn(
          `❌ DUPLICATE CALL BLOCKED: Question "${question}" is already being processed for node ${currentNode.id}`
        );
        return;
      }
      ...
      processingQuestionsRef.current.add(questionKey);
      console.log(
        `✅ Question processing started: "${question}" (key: ${questionKey})`
      );
      ...
      try {
        ...
        console.log(
          `[${request.id}] Question analysis complete: ${phases.length} videos needed`
        );
        console.log(
          `[${request.id}] Phases:`,
          phases.map((p) => p.sub_topic).join(", ")
        );
        ...
        for (let i = 0; i < phases.length; i++) {
          ...
          console.log(
            `[${request.id}] Generating video ${i + 1}/${phases.length}: ${
              phase.sub_topic
            }`
          );
          ...
          if (result.success && result.sections && result.sections.length > 0) {
            ...
            console.log(
              `[${request.id}] ✓ Generated video ${i + 1}: ${phase.sub_topic}`
            );
          } else {
            const errorMsg =
              result.error ||
              `Failed to generate video for: ${phase.sub_topic}`;
            console.error(
              `[${request.id}] Error generating video ${i + 1}:`,
              errorMsg
            );
        ...
        if (generatedItems.length > 0) {
          for (let i = 0; i < generatedItems.length; i++) {
            ...
            console.log(
              `[${request.id}] ✓ Added node ${i + 1} to tree: ${
                phase.sub_topic
              }`
            );
          }
          ...
          setMostRecentGenerationId((current) => {
            if (current === request.id) {
              console.log(
                `[${request.id}] Most recent request - navigating to question branch`
              );
          ...
          console.log(
            `[${request.id}] ✓ Question branch created with ${generatedItems.length} videos`
          );
        } else {
        ...
        processingQuestionsRef.current.delete(questionKey);
        console.log(
          `✓ Question processing completed and cleaned up: "${question}" (key: ${questionKey})`
        );
      }
    },
      ...
      try {
        ...
        if (correct) {
          ...
          try {
            ...
            const videoResult = await generateVideoScenes(
              explanationTopic,
              (progress) => {
                setGenerationProgress(progress);
              },
            ...
            if (
              videoResult.success &&
              videoResult.sections &&
              videoResult.sections.length > 0
            ) {
              ...
              setError(
                videoResult.error || "Failed to generate explanation video"
              );
            }
          } catch (err) {
            const errorMsg =
              err instanceof Error
                ? err.message
                : "Unknown error generating explanation";
            setError(errorMsg);
            onError?.(errorMsg);
          } finally {
            setIsGenerating(false);
  ...
  const createQuestionNode = useCallback(
    async (leafNodeId: string) => {
      try {
        ...
        const hasQuestionNode = children.some(
          (child) => child.segment.isQuestionNode
        );
        if (hasQuestionNode) {
        ...
        const questionNode = addChildNode(
          session.tree,
          leafNodeId,
          questionSegment,
          "Question"
        ...
        const errorMsg =
          err instanceof Error
            ? err.message
            : "Unknown error creating question node";
        setError(errorMsg);
        onError?.(errorMsg);
      } finally {
        setIsGenerating(false);
   ...
   */
  const handleLeafQuestionAnswer = useCallback(
    async (
      question: string,
      answer: string
    ): Promise<{
   ...
   */
  const generateFollowUpVideos = useCallback(
    async (
      wasCorrect: boolean,
      question?: string,
      userAnswer?: string,
      ...
      try {
        console.log(
          `Generating follow-up videos (answer was ${
        ...
        const { generateAdaptiveTopic } = await import(
          "../services/llmService"
        );
        ...
        if (!topicResult.success || !topicResult.topic) {
          const errorMsg =
            topicResult.error || "Failed to generate adaptive topic";
          setError(errorMsg);
          onError?.(errorMsg);
          setIsGenerating(false);
          return;
        ...
        if (result.success && result.sections && result.sections.length > 0) {
          ...
          console.log(
            `Generated ${segments.length} follow-up videos for adaptive topic: ${adaptiveTopic}`
          );
        } else {
          const errorMsg =
            result.error || "Failed to generate follow-up videos";
          setError(errorMsg);
          onError?.(errorMsg);
        }
      } catch (err) {
